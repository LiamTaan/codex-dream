...
- 安装器在替换受管运行时目录前停止已记录的旧注入器，支持开发版、旧版和覆盖安装之间的安全交接。
- 启动清理对短暂 PID/launchd 切换进行一次有界重试，持续身份不匹配仍会安全失败。
- 应用完成状态增加显式 CDP 通知与状态文件轮询兜底，避免进度层滞留。
- 修复 macOS 进程名被截断时的 Codex 运行状态误判。
