...
# Codex process: inspect the executable path instead of `pgrep -x`. On macOS
# the process comm field can be truncated (for example `/Applications/Ch`),
# so an exact name probe incorrectly reports a running Codex as stopped.
if /bin/ps -axo command= 2>/dev/null \
  | /usr/bin/awk '$1 ~ /\/Contents\/MacOS\/(ChatGPT|Codex)$/ { found=1 } END { exit(found ? 0 : 1) }'; then
  CODEX_RUNNING="true"
fi
