...
function setBusy(busy) { $$('button').forEach((button)=>{ const action=button.dataset.action; button.disabled=busy||Boolean(latestStatus&&!latestStatus.available&&action&&action!=="install"); }); }
function renderStatus(status) {
  ...
  primaryButton.dataset.action=status.available?"start":"install";
  primaryButton.textContent=status.available?"应用皮肤":"安装运行时";
...
async function action(actionName){ setBusy(true); try { if(actionName==="customize") await window.dreamSkin.chooseImage(); else await window.dreamSkin.performAction(actionName); toast(actionName==="install"?"运行时安装完成":actionName==="restore"?"已请求恢复官方外观":"操作已发送"); setTimeout(refresh,1200); } catch(error){ toast(error.message,true); } finally { setBusy(false); } }
